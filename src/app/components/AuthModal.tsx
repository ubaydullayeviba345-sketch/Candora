...
export default function AuthModal() {
  const { authOpen, authTab, closeAuth, openAuth, login, register, resetPassword, loginWithGoogle, loginWithApple, lang } = useApp();
  const t = useT(lang);
  ...
  const reset = () => {
    ...
    setPhone("+998 "); setError(""); setNotice(""); setShowPass(false);
  };
  ...
  const handleApple = async () => {
    setError("");
    setLoading(true);
    const res = await loginWithApple().catch(() => ({ error: t.auth.errorGeneral }));
    setLoading(false);
    if (res.error) setError(res.error);
  };
            ...
            <div className="w-full max-w-md bg-card border border-border rounded-3xl shadow-2xl overflow-hidden">
              ...
              <div className="px-6 pb-6 space-y-4">
                ...
                <div className="grid grid-cols-2 gap-3">
                  ...
                  <button onClick={handleApple} disabled={loading} className="flex items-center justify-center gap-2 py-2.5 rounded-xl border border-border hover:bg-muted transition-colors text-sm font-medium disabled:opacity-50">
                    <svg width="18" height="18" viewBox="0 0 24 24" fill="currentColor"><path d="M17.05 20.28c-.98.95-2.05.8-3.08.35-1.09-.46-2.09-.48-3.24 0-1.44.62-2.2.44-3.06-.35C2.79 15.25 3.51 7.7 9.05 7.4c1.39.07 2.35.76 3.16.8 1.21-.24 2.37-.96 3.67-.84 1.57.13 2.75.77 3.52 1.97-3.23 1.94-2.47 5.84.54 7.23-.65 1.6-1.49 3.19-2.89 3.72zM12.03 7.25c-.16-2.57 1.88-4.67 4.34-4.85.32 2.91-2.61 5.09-4.34 4.85z"/></svg>
                    {t.auth.apple}
                  </button>
                </div>
